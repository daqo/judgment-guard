...
- The five output sections in `SKILL.md`, `templates/report_template.md`, and `examples/worked-example.md` must stay in sync. The Evidence sweep block (per load-bearing claim: verdict, then one bullet per query with a `(+) (−) (↻) (⇄)` marker) and the Coverage note live inside the Claim map section, not as extra sections. The per-result ledger stays internal
